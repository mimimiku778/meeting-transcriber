...
OBS等で録画したGoogle Meet/Teams/Discord等の会議動画を入力として（準備するのは動画だけ）:
1. mlx-Whisperで文字起こし（Apple Silicon最適化）
2. speakrs(CoreML)で話者分離＋声紋で話者同一性を解決
3. 動画フレーム（一括・並列OCR）から参加者・共有資料を確認
4. 案件ストアの組織図/帰属ルール/取捨を適用して議事録を生成、ユーザーの修正から学習
