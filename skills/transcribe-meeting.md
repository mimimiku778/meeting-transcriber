...
話者識別:
1. pyannote.audio v2（高精度）← デフォルト
2. simple-diarizer（従来版 --diarization-v1。huggingface_hub 1.x では非互換の場合あり）

番号を入力（Enterで pyannote.audio v2）:
```
...
2. 無い／不完全なら、フレームから以下を確定して作成する（`templates/project.context.template.yaml` を雛形に）:
   - **参加者パネル**のフレーム（冒頭）を Read し、各タイルの氏名と**会社プレフィックス**を読む
     （例「DC 根本敬太」=DGCircus、プレフィックス無し=別テナント＝別会社）。
...
3. **意味反転の罠に注意**: 「こっち側＝自社」だからといって、会議の進行役＝自社とは限らない。
   ...
   読み替えない**（例: 「DCさん」を推測でDDに変えない）。判断はフレームの確定事実が最優先。
...
IAP/Ib/IAV→IAB様、GCさん→DCさん、トーマナ→トンマナ 等の**確実な表記ゆれのみ**を置換する。
DC など意味反転しうる略号や、確証の無い語（リリーガ等）は**触らない**。
...
3. **Stage-2 owner解決**: 各 action_item / 一部 decision に担当会社を **DC / DD / IAB / UNKNOWN の
   いずれか**で付与（`project.context.yaml` の `attribution_rules` に従う）。各アイテムに必ず:
...
```
## 決定事項
- [D1] 画像を496×496化、アプリ改修なしで新デザイン反映（根拠: 発話者3 02:44「理解あってますか」→合意）

## 宿題（アクションアイテム）
| # | 内容 | 担当 | 期限 | 根拠（話者/時刻/引用） | 確度 |
|---|------|------|------|----------------------|------|
| A1 | 翌日IABにUI案(推奨パターン①/前回案)を確認 | DD | 翌日 | 発話者3 04:13「明日IAB様に確認をとります」 | high |
| A2 | Excel(商品feature)の見切れ修正・17番、納品まで | DC | 納品まで | 発話者2 19:02「わかりました」 | high |

## 課題（未決・要検討）
- [I1] Mac/WindowsでExcel図形の見切れ差異（要再現確認）
```
...
- **略号を額面で読み替えない**: 「DCさん」等は意味反転しうる。フレームで確定した対応に従い、
